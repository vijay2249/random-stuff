...
#define INITIAL_PROMPT "Lab2_Assignment- Mini Linux Terminal" //initial display message
...
int Command_Execution(char *argv[]){
    ...
    if(strcmp(argv[0], "clear") == 0){
        system("clear"),printf("%s", INITIAL_PROMPT);
        flag = false;
    }
        ...
        if (argv[1] == NULL) chdir(getenv("HOME")); //if nothing mentioned then change to home
        else chdir(argv[1]);
        flag = false;
    }
    if(flag){
